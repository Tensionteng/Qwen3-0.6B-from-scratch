...
class MiniMindForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **args,
        ...
        hidden_state, past_kvs, aux_loss = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
        ...
        if labels is not None:
            # 在因果语言模型中，通常需要将 logits 和 labels 错一位进行比较
            # logits 的形状: (batch_size, seq_len, vocab_size)
            # labels 的形状: (batch_size, seq_len)
        ...
        return CausalLMOutputWithPast(
            ...
            hidden_states=hidden_state,  # hidden_states is not returned by default
        )

    # --- GenerationMixin 需要这个方法来准备模型的输入 ---
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, **kwargs):
        # 这个方法主要用于 .generate() 函数，实现 KV Caching
        model_inputs = {"input_ids": input_ids}

        if past_key_values is not None:
            model_inputs["past_key_values"] = past_key_values

        # 确保 attention_mask 也被正确传递
        model_inputs["attention_mask"] = kwargs.get("attention_mask", None)
        model_inputs["use_cache"] = kwargs.get(
            "use_cache", True
        )  # 在生成时通常使用cache
